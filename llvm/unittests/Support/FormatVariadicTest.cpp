...
TEST(FormatAndFormatvTest, EquivalentHexFormatting) {
  ...
  EXPECT_EQ("0x00000000ff", printToString(100, format("0x%*.*x", HexDigits, HexDigits, N)));
  
  // Now, do the same with formatv()
  EXPECT_EQ("0x00000000ff", formatv("0x{0:x-}", fmt_align(N, AlignStyle::Right, HexDigits, '0')).str());
}
