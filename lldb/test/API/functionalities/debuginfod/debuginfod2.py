...
def ServeDirectoryWithHTTP(directory="./", port_range_to_try=range(8000,8010), hostname='localhost'):
  ...
  for port in port_range_to_try:
    ...
    # of the block.
    with TCPServer(server_address=(hostname, port), RequestHandlerClass=Handler, bind_and_activate=False) as httpd:
      ...
      try:
        ...
        print(time.asctime(), "Address already in use:", httpd.server_address)
        continue

      if not address_free:
        continue

      print(time.asctime(), "Serving files from ", directory, " on ", httpd.server_address)
...
if __name__ == '__main__':
  try:
    ServeDirectoryWithHTTP('/opt/notnfs/kkleine/debuginfod-test-data/buildroot/', port_range_to_try=range(8000,8010))
